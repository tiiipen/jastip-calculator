...
// =========================================================================
const WEBHOOK_URL = "https://ristretto-flow.tiiipen.com/webhook/member-verification"; 
...
async function checkCredentials() {
    ...
    try {
        ...
        // SINKRONISASI: Panggil resize saat error muncul
...
// =========================================================================
// --- JASTIP APP FUNCTIONS (ZERO BUFFER HEIGHT) ---
// =========================================================================
...
// SINKRONISASI LOGIKA AUTO ADJUSTER DENGAN MARKETPLACE APP (ZERO BUFFER)
function getAccurateHeight() {
    ...
    // Logika 2: Aplikasi Utama Jastip
    const container = document.getElementById('jastip-app'); 
...
function setupSync(sId, iId) { const s=document.getElementById(sId), i=document.getElementById(iId); if(s&&i){ s.addEventListener('input',()=>{i.value=s.value;}); i.addEventListener('input',()=>{let v=parseFloat(i.value); if(v>s.max)v=v>s.max?s.max:v; if(v<s.min)v=v<s.min?s.min:v; s.value=v;}); } }
...
function calculateJastip() {
    ...
    // Panggil resize sequence setelah semua konten hasil kalkulasi muncul
    triggerResizeSequence(); 
}
